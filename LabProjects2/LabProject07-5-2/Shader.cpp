...
void CInstancingShader::BuildObjects(ID3D11Device *pd3dDevice)
{
	...
	pSphereMesh->AssembleToVertexBuffer(1, &m_pd3dSphereInstanceMatrixBuffer, &m_nInstanceMatrixBufferStride, &m_nInstanceMatrixBufferOffset);
	pSphereMesh->AssembleToVertexBuffer(1, &m_pd3dSphereInstanceColorBuffer, &m_nInstanceColorBufferStride, &m_nInstanceColorBufferOffset);
}
...
void CInstancingShader::Render(ID3D11DeviceContext *pd3dDeviceContext, CCamera *pCamera)
{
	...
	for (int j = 0; j < nCubeObjects; j++) D3DXMatrixTranspose(&pd3dxmtxCubes[j], &m_ppObjects[j]->GetmtxWorld());
	...
	for (int j = nCubeObjects; j < m_nObjects; j++) D3DXMatrixTranspose(&pd3dxmtxSpheres[j - nCubeObjects], &m_ppObjects[j]->GetmtxWorld());
