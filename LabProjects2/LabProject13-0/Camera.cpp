...
//CCamera 클래스의 GenerateViewMatrix() 멤버 함수를 다음과 같이 정의한다.
//이 함수는 카메라 변환 행렬을 생성한다.
//카메라의 위치 벡터,카메라가 바라보는 지점, 카메라의 Up 벡터(로컬 y - 축 벡터)를
//파라메터로 사용하는 D3DXMatrixLookAtLH() 함수를 사용한다.
/*카메라 변환 행렬을 생성하는 함수이다. 카메라의 위치 벡터, 카메라가 바라보는 지점, 카메라의 Up 벡터(로컬 y-축 벡터)를 파라메터로 사용하는 D3DXMatrixLookAtLH() 함수를 사용한다.*/

void CCamera::GenerateViewMatrix()
{
	D3DXMatrixLookAtLH(&m_d3dxmtxView, &m_d3dxvPosition,
		&m_pPlayer->GetPosition(), &m_d3dxvUp);
}
...
/*카메라 변환 행렬과 투영 변환 행렬을
쉐이더로 전달하기 위한 구조체에 대한 상수 버퍼를 생성하는 함수이다.*/
...
/*카메라 변환 행렬과 투영 변환 행렬을
쉐이더로 전달하기 위한 구조체에 대한 상수 버퍼를 갱신하고
디바이스 컨텍스트의 슬롯에 연결하는 함수이다.*/
...
CFirstPersonCamera::CFirstPersonCamera(CCamera *pCamera) : CCamera(pCamera)
{
	...
	if (pCamera)
	{
		...
		y-좌표가 0.0f가 되도록 한다. 이것은 <그림 8>과 같이 로컬 x-축 벡터와 로컬 z-축 벡터를 xz-평면(지면)으로 투영하는 것을 의미한다.
...
void CCamera::CalculateFrustumPlanes()
{
	...
	for (int i = 0; i < 6; i++) D3DXPlaneNormalize(&m_d3dxFrustumPlanes[i], 
		&m_d3dxFrustumPlanes[i]);
}

/*이 함수는 바운딩 박스(최소점과 최대점)가 절두체의 
어떤 면의 바깥쪽에 있으면 false를 반환하고 그렇지 않으면 true를 반환한다.*/
bool CCamera::IsInFrustum(D3DXVECTOR3& d3dxvMinimum, D3DXVECTOR3& d3dxvMaximum)
{
	...
	for (int i = 0; i < 6; i++)
	{
		...
		근접점은 절두체에 포함되지 않는다. 근접점이 어떤 평면의 바깥(앞)쪽에 
		있으면 근접점을 평면의 방정식에 대입하면 부호가 양수가 된다. 각 평면의 
		방정식에 근접점을 대입하는 것은 근접점 (x, y, z)과 평면의 법선벡터 (a, b, c)의
