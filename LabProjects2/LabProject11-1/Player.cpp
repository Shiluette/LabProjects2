...
//CPlayer 클래스의 CreateShaderVariables() 멤버 함수
void CPlayer::CreateShaderVariables(ID3D11Device *pd3dDevice)
{
	if (m_pCamera) m_pCamera->CreateShaderVariables(pd3dDevice);
	//왜 지워져 있지?
}

//CPlayer 클래스의 UpdateShaderVariables() 멤버 함수
...
CCamera *CPlayer::OnChangeCamera(ID3D11Device *pd3dDevice, DWORD nNewCameraMode, DWORD nCurrentCameraMode)
{
	...
	/*현재 카메라의 모드가 스페이스-쉽 모드의 카메라이고 새로운 카메라가 1인칭 또는 3인칭 카메라이면 플레이어의 Up 벡터를 월드좌표계의 
	y-축 방향 벡터(0, 1, 0)이 되도록 한다. 즉, 똑바로 서도록 한다. 그리고 스페이스-쉽 카메라의 경우 플레이어의 이동에는 제약이 없다. 
	특히, y-축 방향의 움직임이 자유롭다. 그러므로 플레이어의 위치는 공중(위치 벡터의 y-좌표가 0보다 크다)이 될 수 있다. 이때 새로운 카메라가 
	1인칭 또는 3인칭 카메라이면 플레이어의 위치는 지면이 되어야 한다. 그러므로 플레이어의 Right 벡터와 Look 벡터의 y 값을 0으로 만든다. 
...
//CPlayer 클래스의 Render() 함수를 다음과 같이 변경한다.
...
//CTerrainPlayer 클래스의 OnPlayerUpdated() 함수를 다음과 같이 
//정의한다.이 함수는 플레이어의 위치(벡터)가 바뀔 때 마다 호출된다.
void CTerrainPlayer::OnPlayerUpdated(float fTimeElapsed)
{
	...
	/*높이 맵에서 플레이어의 현재 위치 (x, z)의 y 값을 구한다. 그리고 플레이어 
	메쉬의 높이가 12이고 플레이어의 중심이 직육면체의 가운데이므로 y 값에 메쉬의
	높이의 절반을 더하면 플레이어의 위치가 된다.*/
	
	float fHeight = pTerrain->GetHeight(d3dxvPlayerPosition.x, d3dxvPlayerPosition.z, bReverseQuad) + 6.0f;
	//float fHeight = 100.0f + 6.0f;

	/*플레이어의 속도 벡터의 y-값이 음수이면(예를 들어, 중력이 적용되는 경우)
	플레이어의 위치 벡터의 y-값이 점점 작아지게 된다. 이때 플레이어의 현재 
	위치의 y 값이 지형의 높이(실제로 지형의 높이 + 6)보다 작으면 플레이어가
	땅속에 있게 되므로 플레이어의 속도 벡터의 y 값을 0으로 만들고 플레이어의
	위치 벡터의 y-값을 지형의 높이로 설정한다. 그러면 플레이어는 지형 위에 있게 된다.*/
...
//CTerrainPlayer 클래스의 OnCameraUpdated() 함수를 다음과
//같이 정의한다.이 함수는 카메라의 위치(벡터)가 바뀔 때 마다 호출된다.
void CTerrainPlayer::OnCameraUpdated(float fTimeElapsed)
{
	...
	이렇게 되면 <그림 4>의 왼쪽과 같이 지형이 그려지지 않는 경우가 발생한다(카메라가 지형
	안에 있으므로 와인딩 순서가 바뀐다). 이러한 경우가 발생하지 않도록 카메라의 위치의
	최소값은 (지형의 높이 + 5)로 설정한다. 카메라의 위치의 최소값은 지형의 모든 위치에서
