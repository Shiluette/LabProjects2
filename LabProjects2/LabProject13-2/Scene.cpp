...
void CScene::BuildObjects(ID3D11Device *pd3dDevice)
{
	...
	pRedInstancedObjectsShader->BuildObjects(pd3dDevice, GetTerrain(), NULL, pStoneTexture, 0);
	m_ppShaders[2] = pRedInstancedObjectsShader;

	CInstancingShader *pGreenInstancedObjectsShader = new CInstancingShader();
	pGreenInstancedObjectsShader->CreateShader(pd3dDevice);
	pGreenInstancedObjectsShader->BuildObjects(pd3dDevice, GetTerrain(), NULL, pBrickTexture, 1);
	m_ppShaders[3] = pGreenInstancedObjectsShader;

	CInstancingShader *pBlueInstancedObjectsShader = new CInstancingShader();
	pBlueInstancedObjectsShader->CreateShader(pd3dDevice);
	pBlueInstancedObjectsShader->BuildObjects(pd3dDevice, GetTerrain(), NULL, pWoodTexture, 2);
	m_ppShaders[4] = pBlueInstancedObjectsShader;
}
