...
bool CGameFramework::CreateRenderTargetDepthStencilView()
{
	...
	if (FAILED(hResult = m_pDXGISwapChain->
		GetBuffer(0, __uuidof(ID3D11Texture2D), (LPVOID*)&pd3dBackBuffer))) return(false);
	if (FAILED(hResult = m_pd3dDevice->
		CreateRenderTargetView(pd3dBackBuffer, NULL, &m_pd3dRenderTargetView))) return(false);
...
//FAILED? hResult? 
//HRESULT. return true, false. 무슨 의미?
...
bool CGameFramework::CreateDirect3DDisplay()
{
	...
	//::의 의미가 무엇인가? 전역의 의미?
	//ZeroMemory(Memory Allocation, Memory Copy(length))
	
	dxgiSwapChainDesc.BufferCount = 1;// 버퍼카운트?
	dxgiSwapChainDesc.BufferDesc.Width = m_nWndClientWidth;
	dxgiSwapChainDesc.BufferDesc.Height = m_nWndClientHeight;

	dxgiSwapChainDesc.BufferDesc.Format = DXGI_FORMAT_R8G8B8A8_UNORM;
	
	dxgiSwapChainDesc.BufferDesc.RefreshRate.Numerator = 60;//분자
	dxgiSwapChainDesc.BufferDesc.RefreshRate.Denominator = 1;//분모
	dxgiSwapChainDesc.BufferUsage = DXGI_USAGE_RENDER_TARGET_OUTPUT;
	dxgiSwapChainDesc.OutputWindow = m_hWnd;
	dxgiSwapChainDesc.SampleDesc.Count = 1;//샘플 디스크립션?
	...
	HRESULT hResult = S_OK;
	//디바이스의 드라이버 유형과 특성 레벨을 지원하는 디바이스와 
	//스왑 체인을 생성한다. 고수준의 디바이스 생성을 시도하고 실패하면 다음 수준의 디바이스를 생성한다.
...
void CGameFramework::OnProcessingKeyboardMessage(HWND hWnd, UINT nMessageID, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_KEYUP:
		...
		case VK_F2:
		case VK_F3:
			if (m_pPlayer) m_pPlayer->ChangeCamera(m_pd3dDevice, 
				(wParam - VK_F1 + 1), m_GameTimer.GetTimeElapsed());
			...
			//장난질을 쳐놨다. 삼항 연산자로 ? F1인지 묻고, F2, F3... 
			//왜 굳이 이런짓을했나 했는데, 중복되는 부분을 없애기 위해서 였구나.
		{
					  D3DXCOLOR d3dxcColor = (wParam == VK_F1) ?
...
LRESULT CALLBACK CGameFramework::OnProcessingWindowMessage(HWND hWnd, UINT nMessageID, WPARAM wParam, LPARAM lParam)
{
	switch (nMessageID)
	{
		...
		case WM_SIZE:
		{
					...
					if (pCamera) pCamera->SetViewport(m_pd3dDeviceContext, 0, 0, 
...
void CGameFramework::BuildObjects()
{
	...
	//플레이어의 위치가 변경될 때 지형의 정보에 따라 플레이어의
	//위치를 변경할 수 있도록 설정한다.
	m_pPlayer->SetPlayerUpdatedContext(pTerrain);
	//카메라의 위치가 변경될 때 지형의 정보에 따라 
	//카메라의 위치를 변경할 수 있도록 설정한다.
